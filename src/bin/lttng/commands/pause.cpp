...
const struct argpar_opt_descr pause_options[] = {
...
{
	fmt::print("Usage: lttng [GENERAL OPTIONS] pause [SESSION]\n");
}
...
cmd_error_code pause_tracing_session(std::string session_name)
{
	const lttng_error_code pause_ret_code = lttng_pause_session(session_name.c_str());

	if (pause_ret_code != LTTNG_OK) {
		switch (pause_ret_code) {
		case LTTNG_ERR_NO_SESSIOND:
			ERR_FMT("Cannot handle pause request. {}.",
		...
		case LTTNG_ERR_SESSION_DATA_CONSUMPTION_ALREADY_PAUSED:
			WARN_FMT("Session '{}': {}.",
				 session_name,
				 lttng_strerror(LTTNG_ERR_SESSION_DATA_CONSUMPTION_ALREADY_PAUSED));
			return CMD_WARNING;

		default:
			ERR_FMT("Session '{}': Error while attempting to pause data consumption.",
				session_name);
			return CMD_ERROR;
		}
	}

	fmt::print("Session '{}': Paused userspace data consumption.\n", session_name);
 ...
 *  cmd_pause
 *
 *  The `pause [SESSION]` command.
 *
 *  Pausing a recording session stops the consumption of tracing data to
 *  the disk or network. The opposite command, lttng-resume restarts the
 *  consumption of trace data.
 *
...
{
	/* Skip "pause" in the list of arguments */
	...
	argpar_iter = argpar_iter_create(my_argc, my_argv, pause_options);
	if (!argpar_iter) {
	...
	if (session_name.empty()) {
		...
		if (session_name.empty()) {
			ERR_FMT("No tracing session to pause was defined and there "
	...
	res = pause_tracing_session(session_name);
