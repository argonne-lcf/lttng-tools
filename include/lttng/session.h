...
/*!
@brief
    Resumes the recording session named \lt_p{session_name}.

Resuming a recording session restarts the consumption of tracing data to
the disk or network.

@param[in] session_name
    Name of the recording session to resume.
...
@sa lttng_pause_session() --
    Pauses an active recording session.
*/
LTTNG_EXPORT extern enum lttng_error_code lttng_resume_session(const char *session_name);
